...
from diffsynth_engine.config import DiffsynthConfig, ControlType
...
class FluxImagePipeline(BasePipeline):
    ...
    def __init__(
        ...
        super().__init__(
            vae_tiled=config.vae_tiled if config.vae_tiled is not None else False,
            vae_tile_size=config.vae_tile_size if config.vae_tile_size is not None else 256,
            vae_tile_stride=config.vae_tile_stride if config.vae_tile_stride is not None else 256,
        ...
        self.noise_scheduler = RecifitedFlowScheduler(
            shift=config.shift if config.shift is not None else 3.0,
    ...
    @classmethod
    def from_pretrained(cls, config: DiffsynthConfig) -> "FluxImagePipeline":
        if config.vae_path is None:
            config.vae_path = fetch_model("muse/FLUX.1-dev-fp8", path="ae-bf16.safetensors")
        if config.clip_l_path is None and config.load_text_encoder:
            config.clip_l_path = fetch_model("muse/FLUX.1-dev-fp8", path="clip-bf16.safetensors")
        ...
        if config.load_text_encoder:
            ...
            clip_state_dict = cls.load_model_checkpoint(config.clip_l_path, device="cpu", dtype=config.clip_l_dtype)
        ...
        if config.load_text_encoder:
            ...
            with LoRAContext():
                text_encoder_1 = FluxTextEncoder1.from_state_dict(
                    clip_state_dict, device=init_device, dtype=config.clip_l_dtype
                )
        ...
        if config.parallelism > 1:
            cls.init_parallel_config(config)
