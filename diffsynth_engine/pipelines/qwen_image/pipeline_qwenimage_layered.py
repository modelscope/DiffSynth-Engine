# Adapted from https://github.com/huggingface/diffusers/blob/main/src/diffusers/pipelines/qwenimage/pipeline_qwenimage_layered.py
...
the image\n<|vision_start|><|image_pad|><|vision_end|><|im_end|>\n<|im_start|>assistant\n"""
    ...
    def from_pretrained(cls, model_path_or_config: str | QwenImagePipelineConfig):
        """
        Load a QwenImageLayeredPipeline from a pretrained model path or config.

        Args:
            model_path_or_config: Either a string path to the model directory or a QwenImagePipelineConfig instance.

        Returns:
            QwenImageLayeredPipeline: The loaded pipeline.
        """
    ...
    def get_image_caption(self, prompt_image, use_en_prompt: bool = True, device=None):
        """Generate image caption using the text encoder."""
        if use_en_prompt:
    ...
    def _pack_latents(latents, batch_size, num_channels_latents, height, width, layers):
        """Pack latents for layered generation (with layers dimension)."""
    ...
    def _unpack_latents(latents, height, width, layers, vae_scale_factor):
        """Unpack latents from layered generation (with layers dimension)."""
    ...
    def prepare_latents(
        ...
        """
        Prepare latents for layered denoising.
    ...
    def __call__(
        ...
        Args:
            ...
            true_cfg_scale (`float`, *optional*, defaults to 4.0):
                Guidance scale for classifier-free guidance. Enabled by setting `true_cfg_scale > 1` and providing
                a `negative_prompt`.
            layers (`int`, *optional*, defaults to 4):
                Number of layers to decompose the image into.
            ...
            cfg_normalize (`bool`, *optional*, defaults to `False`):
                Whether to enable CFG normalization.
        ...
        Returns:
            ...
            [`QwenImagePipelineOutput`] if `return_dict` is True, otherwise a `tuple`. When
