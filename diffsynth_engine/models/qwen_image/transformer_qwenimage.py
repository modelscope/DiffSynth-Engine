...
# the script injects a profiler via register_multi_block_profiler(),
# and the block loop below automatically starts/steps/stops around the
# first 4 blocks (index 0-3). All profiling parameters live in the script
# — the model code is kept clean.
...
def register_multi_block_profiler(profiler):
    ...
    automatically start before transformer block 0, step after each of the
    first 4 blocks (0-3), and stop after block 3, capturing operator data
    from 4 distinct blocks.
    ...
    Example in script:
        profiler = torch_npu.profiler.profile(
            activities=[...],
            schedule=torch_npu.profiler.schedule(wait=0, warmup=0, active=4),
...
def reset_multi_block_profiler():
    ...
    Call this after warmup or when you want to re-run profiling within the
    same process.
    """
...
class QwenEmbedRope(nn.Module):
    def __init__(self, theta: int, axes_dim: List[int], scale_rope=False):
        ...
        # compatible with model initialization on meta device
...
class QwenDoubleStreamAttention(nn.Module):
    """
    Attention module for Qwen double-stream architecture, matching DoubleStreamLayerMegatron logic.
    ...
    def forward(
        ...
        if encoder_hidden_states is None:
            raise ValueError("QwenDoubleStreamAttention requires encoder_hidden_states (text stream)")
        ...
        # Reshape for multi-head attention: [B, S, H*D] -> [B, S, H, D]
        ...
        for module in self.to_out:
            img_attn_output = module(img_attn_output)
...
class QwenImageTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        img_attn_output, txt_attn_output = self.attn(
            hidden_states=img_modulated,  # Image stream
            encoder_hidden_states=txt_modulated,  # Text stream
...
class QwenImageTransformer2DModel(DiffusionModel):
    """
    ...
    def forward(
        ...
        Args:
            ...
            attention_kwargs (`dict`, *optional*):
                A kwargs dictionary that if specified is passed along to the attention layer.
