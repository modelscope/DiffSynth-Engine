# Adapted from https://github.com/huggingface/diffusers/blob/main/src/diffusers/models/autoencoders/autoencoder_kl_qwenimage.py
...
class AutoencoderKLQwenImage(DiffusionModel):
    r"""
    ...
    def enable_vae_parallel(self):
        r"""
        Enable VAE parallel processing. When this option is enabled, the VAE will split the input tensor across
        multiple devices to perform encoding and decoding in parallel for better performance.
        """
        self.use_vae_parallel = True

    def disable_vae_parallel(self):
        r"""
        Disable VAE parallel processing. If `enable_vae_parallel` was previously enabled, this method will go back to
        computing encoding and decoding on a single device.
        """
        self.use_vae_parallel = False

    def clear_cache(self):
        r"""Clear the feature cache for causal convolutions."""
    ...
    def _encode(self, x: torch.Tensor):
        ...
        if self.use_tiling and (width > self.tile_sample_min_width or height > self.tile_sample_min_height):
            if self.use_vae_parallel:
                return self.parallel_tiled_encode(x)
    ...
    def _build_tile_mask(self, tile, is_bound, border):
    ...
    def _build_tile_list(self, H, W, size_h, size_w, stride_h, stride_w):
        """Build a row-major nested list of (h, h_, w, w_) tiles."""
        tiles = []
        for h in range(0, H, stride_h):
            if h - stride_h >= 0 and h - stride_h + size_h >= H:
                continue  # covered by previous row
            for w in range(0, W, stride_w):
                if w - stride_w >= 0 and w - stride_w + size_w >= W:
                    continue  # covered by previous column
                tiles.append((h, h + size_h, w, w + size_w))
        return tiles

    def parallel_tiled_encode(self, x: torch.Tensor) -> torch.Tensor:
        r"""Encode a batch of images using a tiled encoder distributed across multiple devices.

        Tiles are assigned to ranks via round-robin and blended with mask-weighted averaging
        followed by an all_reduce to gather contributions from all ranks.
        ...
        blend_height = tile_latent_min_height - self.tile_sample_stride_height // self.spatial_compression_ratio
        blend_width = tile_latent_min_width - self.tile_sample_stride_width // self.spatial_compression_ratio
        ...
        values = torch.zeros(
            x.shape[0], self.z_dim * 2, frame_range, latent_height, latent_width,
            dtype=x.dtype, device=x.device,
        )
        weight = torch.zeros(
            1, 1, frame_range, latent_height, latent_width,
            dtype=x.dtype, device=x.device,
        )

        for i, (h, h_, w, w_) in enumerate(tiles):
            ...
            for k in range(frame_range):
                ...
                if k == 0:
                    ...
                    chunk = x[:, :, 1 + 4 * (k - 1) : 1 + 4 * k, h:h_, w:w_]
                chunk = self.encoder(chunk, feat_cache=self._enc_feat_map, feat_idx=self._enc_conv_idx)
                chunk = self.quant_conv(chunk)
                time.append(chunk)
            encoded_tile = torch.cat(time, dim=2)

            mask = self._build_tile_mask(
                encoded_tile,
                is_bound=(h == 0, h_ >= height, w == 0, w_ >= width),
                border=(blend_height, blend_width),
            )

            th = h // self.spatial_compression_ratio
            tw = w // self.spatial_compression_ratio
            values[:, :, :, th : th + encoded_tile.shape[3], tw : tw + encoded_tile.shape[4]] += encoded_tile * mask
            weight[:, :, :, th : th + encoded_tile.shape[3], tw : tw + encoded_tile.shape[4]] += mask
    ...
    def parallel_tiled_decode(self, z: torch.Tensor, return_dict: bool = True) -> DecoderOutput | torch.Tensor:
        r"""Decode a batch of images using a tiled decoder distributed across multiple GPUs.
        ...
        values = torch.zeros(
            z.shape[0], 3, num_frames, sample_height, sample_width,
            dtype=z.dtype, device=z.device,
        )
        weight = torch.zeros(
            1, 1, num_frames, sample_height, sample_width,
            dtype=z.dtype, device=z.device,
        )
        ...
        for i, (h, h_, w, w_) in enumerate(tiles):
            ...
            for k in range(num_frames):
                self._conv_idx = [0]
                chunk = z[:, :, k : k + 1, h:h_, w:w_]
                chunk = self.post_quant_conv(chunk)
                chunk = self.decoder(chunk, feat_cache=self._feat_map, feat_idx=self._conv_idx)
                time.append(chunk)
            ...
            th = h * self.spatial_compression_ratio
            tw = w * self.spatial_compression_ratio
            values[:, :, :, th : th + decoded_tile.shape[3], tw : tw + decoded_tile.shape[4]] += decoded_tile * mask
            weight[:, :, :, th : th + decoded_tile.shape[3], tw : tw + decoded_tile.shape[4]] += mask
