...
def init_world_group(ranks: List[int], local_rank: int, backend: str) -> GroupCoordinator:
    timeout = timedelta(seconds=IDLE_TIMEOUT_SEC)
...
def init_sp_group(group_ranks: List[List[int]], rank: int, ulysses_degree: int, ring_degree: int, backend: str):
    ulysses_group, ring_group = None, None

    def indices_to_ranks(ranks: List[int], indices: List[int]) -> List[int]:
        return [ranks[i] for i in indices]

    for ranks in group_ranks:
        for i in range(ring_degree):
            indices = [i * ulysses_degree + j for j in range(ulysses_degree)]
            ulysses_ranks = indices_to_ranks(ranks, indices)
            group = torch.distributed.new_group(ranks=ulysses_ranks, backend=backend)
            if rank in ulysses_ranks:
                ulysses_group = group

        for i in range(ulysses_degree):
            indices = [i + j * ulysses_degree for j in range(ring_degree)]
            ring_ranks = indices_to_ranks(ranks, indices)
            group = torch.distributed.new_group(ranks=ring_ranks, backend=backend)
...
def initialize_model_parallel(
    ...
    ulysses_group, ring_group = init_sp_group(
